    ...
    def setup_tools(self):
        ...
        async def qdrant_find(
            ...
            collection_name: Annotated[
                str,
                Field(
                    description="Name of the existing collection to search within. Must be a valid, existing collection name. Non-existent collections will return 'No information found' message."
                ),
            ],
        ) -> List[str]:
            """
            ...
            if not entries:
                return [
                    f"No information found for the query '{query}' in collection {collection_name}"
                ]
        ...
        async def qdrant_collection_info(
            ...
            collection_name: Annotated[
                str,
                Field(
                    description="Name of the collection to inspect and get detailed information about. Can be any string - will return error message if collection doesn't exist or is inaccessible. Returns status, document count, vector configuration, optimization settings, and model information."
                ),
            ],
        ) -> str:
            """
