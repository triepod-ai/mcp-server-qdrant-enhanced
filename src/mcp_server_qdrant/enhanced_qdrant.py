...
class EnhancedQdrantConnector:
    ...
    async def store(self, entry: Entry, *, collection_name: Optional[str] = None):
        ...
        print(
            f"[DEBUG] enhanced_qdrant.py: Starting store for collection '{collection_name}'",
            file=sys.stderr,
        )
        embeddings = await self._embedding_provider.embed_documents(
            [entry.content], collection_name
        )

        # Get collection-specific vector name
        vector_name = self._embedding_provider.get_vector_name(collection_name)
        print(
            f"[DEBUG] enhanced_qdrant.py: Using vector_name '{vector_name}' for store in collection '{collection_name}'",
            file=sys.stderr,
        )
    ...
    async def bulk_store(
        ...
        *,
        ...
        return {
            ...
            "collection_name": collection_name,
            "vector_model": self._embedding_provider.get_model_info_for_collection(
                collection_name
            ),
        }
    ...
    async def search(
        ...
        print(
            f"[DEBUG] enhanced_qdrant.py: Starting search for collection '{collection_name}'",
            file=sys.stderr,
        )
        ...
        vector_name = self._embedding_provider.get_vector_name(collection_name)
        print(
            f"[DEBUG] enhanced_qdrant.py: Using vector_name '{vector_name}' for collection '{collection_name}'",
            file=sys.stderr,
        )
        ...
        try:
            ...
            logger.error(
                f"Search failed for query '{query}' in collection '{collection_name}': {e}"
            )
            raise
    ...
    async def _search_with_retry(
        ...
        for attempt in range(max_retries):
            try:
                ...
                error_msg = str(e).lower()
                print(
                    f"[ERROR] enhanced_qdrant.py: Search failed on attempt {attempt + 1}: {e}",
                    file=sys.stderr,
                )
                print(
                    f"[ERROR] enhanced_qdrant.py: Collection: {collection_name}, Vector name: {vector_name}",
                    file=sys.stderr,
                )
                ...
                if attempt == max_retries - 1:
                    ...
                    if "vector name" in error_msg or "using" in error_msg:
                        print(
                            "[ERROR] enhanced_qdrant.py: Vector name mismatch detected!",
                            file=sys.stderr,
                        )
                        print(
                            f"[ERROR] enhanced_qdrant.py: Collection '{collection_name}' requires explicit vector name. "
                            f"Expected: Check collection config vs actual usage. "
                            f"Current model uses vector name: '{vector_name}'. "
                            f"Verify collection was created with this vector name.",
                            file=sys.stderr,
                        )
                    raise e
    ...
    async def _ensure_collection_exists(self, collection_name: str):
        ...
        if not collection_exists and self._qdrant_settings.auto_create_collections:
            ...
            if "legal" in collection_name.lower():
                ...
                "solutions" in collection_name.lower()
                or "patterns" in collection_name.lower()
            ):
    ...
    async def get_collection_info(self, collection_name: str) -> Dict[str, Any]:
        ...
        try:
            collection_info = await self._client.get_collection(collection_name)
            model_info = self._embedding_provider.get_model_info_for_collection(
                collection_name
            )
    ...
    async def delete_points(
        self, point_ids: List[str], collection_name: str
    ) -> Dict[str, Any]:
