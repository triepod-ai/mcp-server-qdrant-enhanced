...
def is_valid_entry_payload(payload: Dict[str, Any]) -> TypeGuard[Dict[str, Any]]:
    ...
    """
    return (
        isinstance(payload.get("document"), str) and
        len(payload["document"].strip()) > 0 and
        isinstance(payload.get("metadata"), (dict, type(None)))
    )
...
def is_valid_search_result(result: Any) -> TypeGuard[Dict[str, Any]]:
    ...
    """
    return (
        hasattr(result, 'payload') and
        hasattr(result, 'score') and
        isinstance(result.score, (int, float)) and
        is_valid_entry_payload(result.payload)
    )
